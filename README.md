...
Reusable event-driven infrastructure powering two projects:

1. **RAG Knowledge Agent** — OAuth-synced Notion/Jira retrieval agent with memory + citation tracing. **Built** — OAuth → sync → embed → retrieve → streaming answer, for both sources, with a full dashboard.
2. **AI Code Review Bot** — GitHub webhook-triggered style/security analysis. **Not started** — the backbone (queue, idempotency, observability) is ready for it, no project-specific code exists yet.

Both projects share this backbone: `webhook/sync event -> queue -> worker -> LLM call -> structured output -> API -> dashboard`.
Only the event source and the worker's task logic differ between projects.
...
```
  ...
  GitHub Webhook ────►│             │      Knowledge Base · Activity
                      └──────┬──────┘
...
| `services/worker` | Python (aio-pika consumer) | Pulls events off the queue, runs domain logic (fetch → chunk → embed), writes results |
...
- **Activity** — a live (5s-polled) feed of the shared `event_log` table: every sync/embed event and its status (`received` → `processing` → `succeeded`/`failed`/`dead_lettered`), with retry backoff visible when something fails.
...
The RAG agent works with partial credentials too — e.g. Voyage + Google keys alone let `/query` run (with an empty-results fallback until something's synced), and Notion or Jira alone is enough to exercise the full connect → sync → ingest → query loop without both.
...
- **Full loop**: follow the [end-to-end workflow](#end-to-end-workflow) above with real Notion/Jira credentials.
...
## Next steps (project-specific work)

- **Code review bot**: `services/worker/app/tasks/analyze_diff.py`, GitHub App auth + webhook signature verification, a findings schema/table (as `infra/postgres/003_*.sql`).
